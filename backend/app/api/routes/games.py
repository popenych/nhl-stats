...
async def get_game(game_id: int, db: DbSession, _user: CurrentUser) -> object:
    try:
...
async def create_game(data: GameCreate, db: DbSession, user: CurrentUser) -> object:
    return await game_service.create_game(db, data, created_by_user_id=user.id)
...
async def update_game(game_id: int, data: GameUpdate, db: DbSession, user: CurrentUser) -> object:
    ...
    if not game_service.can_edit_game(user, game):
        raise HTTPException(
            ...
            detail="Only a participant in this game (or an admin) can edit it",
        )
...
async def delete_game(game_id: int, db: DbSession, user: CurrentUser) -> None:
    ...
    if not game_service.can_edit_game(user, game):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only a participant in this game (or an admin) can delete it",
        )
